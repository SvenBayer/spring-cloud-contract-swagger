 ...
 * @author Sven Bayer
 */
class XExampleSwaggerContractSpec extends Specification {
    ...
    def "should convert from swagger with json x-example to contract"() {
        given:
        File singleSwaggerYaml = new File(SwaggerContractConverterSpec.getResource("/swagger/x-example/json_xexample_param_swagger.yml").toURI())
    ...
    def "should convert from swagger with json x-example and no ref to contract"() {
        given:
        File singleSwaggerYaml = new File(SwaggerContractConverterSpec.getResource("/swagger/x-example/json_xexample_param_no_ref_swagger.yml").toURI())
        Contract expectedContract = Contract.make {
            label("takeoff_coffee_bean_rocket")
            name("1_takeoff_POST")
            description("API endpoint to send a coffee rocket to a bean planet and returns the bean planet.")
